...
class SignLanguageInterpreter:
    ...
    def match_sign(self, processed_frame):
        """Match the processed frame against known signs."""
        if not self.signs:
            return None, None
            
        # Extract ROI (Region of Interest)
        roi = processed_frame[100:300, 100:300]
        
        best_match = None
        best_score = float('inf')
        
        # Compare with each stored sign
        for sign_name, template in self.signs.items():
            # Ensure template and ROI have the same size
        ...
        # Convert score to confidence (lower MSE = higher confidence)
        if best_score != float('inf'):
            confidence = max(0, 100 - (best_score / 100))  # Scale score to 0-100%
        ...
        while True:
            ...
            if choice == '1':
                ...
                while True:
                    ...
                    matched_sign, confidence = self.match_sign(processed)
                    if matched_sign:
                        cv2.putText(frame, f"Sign: {matched_sign} ({confidence:.2f}%)", 
                                  (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
